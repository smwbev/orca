...
// Why: a table (not an if-chain) so adding an agent is one entry; Claude, Trae, and
// Prime Agent share the same `--print` one-shot contract, Ante's `--prompt` form
...
> = {
  ...
  'prime-agent': isPrintModeHeadlessOneShotCommand,
